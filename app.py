...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://phillipbrown@localhost:5432/pkbroHW5"
## Provided:
...
class UpdatePriority(FlaskForm):
    priority = FloatField('What is the new priority of this item?', validators=[Required()])
    submit = SubmitField("Update")
...
def update(item):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash("***Updated priority of item: " + i.description + "***")
        return redirect(url_for('all_lists'))


    return render_template('update_item.html', form=form, item=i)
...
def delete(lst):
    ...
    flash("Successfully deleted " + lst)
    return redirect(url_for('all_lists'))
